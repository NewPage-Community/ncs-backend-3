// from: github.com/tmc/grpc-websocket-proxy
...
func (p *Proxy) proxy(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		p.logger.Warn("error upgrading websocket:", err)
		return
	}
	defer conn.Close()
	...
	if err != nil {
		p.logger.Warn("error preparing request:", err)
		return
	}
		...
		for {
			...
			if err != nil {
				if isClosedConnError(err) {
					p.logger.Debug("[read] websocket closed:", err)
					return
				}
				p.logger.Warn("error reading websocket message:", err)
				return
			}
			p.logger.Debug("[read] read payload:", string(payload))
			p.logger.Debug("[read] writing to requestBody:")
			n, err := requestBodyW.Write(payload)
			requestBodyW.Write([]byte("\n"))
			p.logger.Debug("[read] wrote to requestBody", n)
			if err != nil {
				p.logger.Warn("[read] error writing message to upstream http server:", err)
				return
			}
		}
	}()
	...
	for scanner.Scan() {
		if len(scanner.Bytes()) == 0 {
			p.logger.Warn("[write] empty scan", scanner.Err())
			continue
		}
		p.logger.Debug("[write] scanned", scanner.Text())
		if err = conn.WriteMessage(websocket.TextMessage, scanner.Bytes()); err != nil {
			p.logger.Warn("[write] error writing websocket message:", err)
			return
		}
	}
	if err := scanner.Err(); err != nil {
		p.logger.Warn("scanner err:", err)
	}
}
